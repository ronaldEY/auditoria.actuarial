...
nombreBD <- "N-6-2 Reservas técnicas por primas_previsionales al 31.12.19.xlsx"
hojaBD <- "N-6-2.1"
...
dfValidador <- data.frame(BD_RRVV_TIT$`Grupo Familiar`, # ID
                          BD_RRVV_TIT$P?liza, # Poliza
                          as.Date(rep(x = fecCierre,nrow(BD_RRVV_TIT)), format = "%d/%m/%Y"), #Periodo
                          BD_RRVV_TIT$`Fecha de devengue`,#Inicio de Vigencia
                          BD_RRVV_TIT$`Fecha de entrada en vigencia de la p?liza`,#Fec. Seleccion de Tablas
                          ...
                          ifelse(substr(BD_RRVV_TIT$`Moneda`,1,3) == "S/.","PEN","USD"),
                          ...
                          0,#BD_RRVV_TIT$`Tasa de Mercado`,#Campo Tasa Mercado 0
...
for(i in 1:filas) {
  ...
  dfBeneficiarios <- BD_RRVV[BD_RRVV$`Grupo Familiar`==dfValidador[i,"ID"] & BD_RRVV$`Relaci?n familiar` != "T" &
