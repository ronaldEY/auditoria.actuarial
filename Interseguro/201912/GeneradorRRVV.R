...
directorioBD <- "C:/Ronald/Auditorias/Interseguro/SBS/122019/RRVV/CIA"
nombreBD <- "RRVV Sura.xlsx"
hojaBD <- "RRVV USD"
...
POR_PG=BD_RRVV%>% group_by(Póliza) %>% summarise(x = sum(`% Pensión`))
...
dfValidador <- data.frame(BD_RRVV_TIT$Póliza,
                          ...
                          BD_RRVV_TIT$`Fecha de envío al MELER`,#Fecha seleccion tabla
                          ...
                          BD_RRVV_TIT$`Fecha de envío al MELER`, # Fecha de Emision
...
for(i in 1:filas) {
  ...
  for(j in 1:benefVigentes) {
    ...
    if(dfBeneficiarios[j,"Parentesco"]==10){
      ...
      dfValidador[i,paste("PORC_HIJ_",contHijos,sep = "")] <- dfBeneficiarios[j,"% Pensión"]/100
      if(dfBeneficiarios[j,"Inválido"] != "N"){
        dfValidador[i,paste("EDAD_MAX_HIJ_",contHijos,sep = "")] <- 111  
      }else if(dfBeneficiarios[j,"Fecha de Solicitud de Pensión"][[1]]<fecHijos28){
...
dfValidador$PORC_PG=if_else(dfValidador$COBERTURA=="SOB",POR_PG$x/100,1)
